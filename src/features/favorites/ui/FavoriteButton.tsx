...
export default function FavoriteButton({ trademarkId }: FavoriteButtonProps) {
  ...
  return (
    <button
      ...
      className={`glass-button flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition ${
        isFavorite
    ...
    >
      <svg
        ...
        className={`h-5 w-5 transition-colors ${isFavorite ? 'fill-current' : 'fill-none'}`}
        viewBox="0 0 24 24"
        stroke="currentColor"
        strokeWidth={2}
      >
      ...
      <span>{isFavorite ? '즐겨찾기 해제' : '즐겨찾기'}</span>
    </button>
  )
}
