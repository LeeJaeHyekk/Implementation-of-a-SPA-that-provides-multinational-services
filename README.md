...
│  ├─ search/             # 검색 페이지
│  │  └─ page.tsx        # 검색 페이지 (SEO 메타데이터)
│  ├─ favorites/          # 즐겨찾기 페이지
│  │  └─ page.tsx        # 즐겨찾기 페이지 (동적 메타데이터)
│  └─ trademark/[id]/     # 상표 상세 페이지
│     └─ page.tsx        # 상세 페이지 (동적 메타데이터)
│
...
│  │  │  ├─ cache/        # 정규화 캐시
│  │  │  ├─ combineFilters.ts
│  │  │  ├─ createFilterChips.ts  # 필터 칩 생성 유틸리티
│  │  │  └─ constants.ts
│  │  ├─ model/           # 상태 관리
│  │  │  ├─ store.ts
│  │  │  ├─ selectors.ts
...
│  │  ├─ fetchTrademarksBase.ts  # 공통 페칭 로직
│  │  ├─ loadLocalJson.ts
│  │  └─ useTrademarksQuery.ts
│  ├─ hooks/               # 공통 React Hooks
│  │  ├─ useSanitizer.ts   # 국가별 Sanitizer 생성 훅
...
│  │  └─ navigation.ts    # 라우터 네비게이션 유틸
│  └─ config/             # 설정
│     ├─ constants.ts     # 프로젝트 상수
│     └─ css-classes.ts   # CSS 클래스 상수
│
├─ data/                   # 로컬 JSON 데이터
...
- **반응형 디자인**: 모바일/태블릿/데스크톱 지원 (Tailwind responsive classes)
- **로딩 상태**: 데이터 로딩 중 스피너 표시 (전체 화면 로딩 지원)
- **에러 처리**: 사용자 친화적인 에러 메시지
- **빈 상태 처리**: 검색 결과 없음, 즐겨찾기 없음 등 상태 표시
- **키보드 네비게이션**: Enter 키 검색, Escape 키 필터/모달 닫기
- **접근성**: ARIA 속성, 시맨틱 HTML, 키보드 지원
...
### 🔍 SEO 최적화
- **페이지별 메타데이터**: 각 페이지에 최적화된 title, description
- **Open Graph 태그**: 소셜 미디어 공유 최적화
- **Twitter Card**: Twitter 공유 최적화
- **동적 메타데이터**: 상표 상세 페이지, 즐겨찾기 페이지 동적 생성
...
- 상수 중앙화 (SEARCH_CONSTANTS, CSS 클래스)
- 컴포넌트 재사용성 향상 (FilterChips, EmptyState, ErrorState)
...
6. `features/search/lib/createFilterChips.ts`에 필터 칩 생성 로직 추가
7. `FilterChips` 컴포넌트가 자동으로 표시됨
...
### 상수 관리
- **SEARCH_CONSTANTS**: 검색 관련 상수 중앙화 (`shared/config/constants.ts`)
  - `VALIDATION_CONFIG`: 검증 설정
  - `DEBOUNCE_DELAY`: Debounce 지연 시간
  - `PAGE_SIZE`: 페이지당 항목 수
  - `LARGE_DATA_THRESHOLD`: 대량 데이터 임계값
  - `CHUNK_SIZE`: 청크 크기
- **CSS 클래스 상수**: 반복되는 CSS 클래스 문자열 통합 (`shared/config/css-classes.ts`)
  - `GLASS_BUTTON_CLASSES`: 버튼 스타일
  - `GLASS_CARD_CLASSES`: 카드 스타일
  - `LAYOUT_CLASSES`: 레이아웃 클래스
  - `GRID_CLASSES`: 그리드 레이아웃
  - `TEXT_CLASSES`: 텍스트 스타일
...
### 모듈화 개선 (v0.1.0)
- ✅ 공통 훅 추출: `useSanitizer`, `usePagination`
- ✅ 필터 칩 컴포넌트화: `FilterChips` 컴포넌트 및 `createFilterChips` 유틸리티
- ✅ 상수 중앙화: `SEARCH_CONSTANTS`, CSS 클래스 상수
- ✅ 공통 UI 컴포넌트: `BackButton`, `NavigateButton`, `EmptyState`, `ErrorState`
- ✅ 네비게이션 유틸리티: `navigation.ts`로 라우터 함수 중앙화
